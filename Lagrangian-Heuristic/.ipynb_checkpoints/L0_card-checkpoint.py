...
@nb.njit(parallel=True)
def mmm2(A,B,index,transpose=False):
    ...
    if not transpose:
        ...
        for i in range(n):
            res[:,i] = mvm(A,B[:,i],index,False)
            
    else: 
        p,n = B.shape
        res = np.zeros((len(index),n),dtype=A.dtype)
        
        for i in range(n):
            res[:,i] = mvm(A,B[:,i],index,True)
...
@nb.njit(cache=True)
def cal_optsize(support, grad, X, grad_supp, alpha, r, lambda2, beta_sub2, p, k):
...
def hard_thresholding_ls(y,X,beta,r,k,alpha,lambda1,lambda2,beta_tilde1,beta_tilde2,L,M=np.inf,sea_max_itr=5):
    ...
    opt_step = cal_optsize(support, grad, X, grad_supp, alpha, r, lambda2, beta_sub2, p, k)
    
    #print("----opt step is",opt_step,"Lchange is",L_change,"L is",1/L)
    if opt_step < L_change:
        beta_new = beta - opt_step*grad
        #print("ori obj is",evaluate_obj(beta,r,alpha,lambda1,lambda2,beta_tilde1,beta_tilde2) )      
        ...
        if p > 5*k:
            ...
            r = y - X@beta_new
        #print("new obj is",evaluate_obj(beta_new,r,alpha,lambda1,lambda2,beta_tilde1,beta_tilde2))     
    else:
...
def Vanilla_IHTCD(y,X,beta,k,alpha=None,lambda1=0.,lambda2=0.,beta_tilde1=None,beta_tilde2=None,L=None,M=np.inf,iht_max_itr=100,ftol=1e-8,
    ...
    while iht_cur_itr < iht_max_itr:
        ...
        search_flag = False
        search_cur_itr = 0
        L_init = L / (1.2**(search_max_itr-1))
        #grad_x = alpha - X.T@r + 2*lambda2*(beta-beta_tilde2)
        while True:
            beta_tmp, r_tmp = hard_thresholding(y,X,beta,r,k,alpha,lambda1,lambda2,beta_tilde1,beta_tilde2,L_init,M)
            f_new = evaluate_obj(beta_tmp,r_tmp,alpha,lambda1,lambda2,beta_tilde1,beta_tilde2)    
            if f_new < f_old: #f_old + (beta_tmp-beta)@grad_x + L_init/2 * np.linalg.norm(beta_tmp-beta)**2 
                beta = np.copy(beta_tmp)
                r = np.copy(r_tmp)
                break
            search_cur_itr += 1
        ...
        f = evaluate_obj(beta,r,alpha,lambda1,lambda2,beta_tilde1,beta_tilde2)
        #print("fold is ",f_old, "fnew is",f, "step", 1/2/L_init)
