...
class GradualPruner:
    ...
    def prune_flop(self,nepochs,lr_schedule,prunepochs,sparsities,flop_ratios,base_level_=0.1,flop_base_=0.1):
        ...
        for epoch in range(self.first_epoch,nepochs):
            ...
            if epoch in prunepochs and (not self.distributed or self.rank == 0):
                ...
                if epoch_index > 0:
                    base_level = sparsities[epoch_index-1]
                    flop_base = flop_ratios[epoch_index-1]
                else:
                    base_level = base_level_
                    flop_base = flop_base_
                self.model.eval()
                w_pruned,mask=self.pruner.prune_flop(self.mask,sparsities[epoch_index],base_level,flop_ratios[epoch_index],flop_base)
                self.mask = mask.to(self.device)
                del mask
                del w_pruned
                pruning_res = self.pruner.results
    ...
    def prune_group(self,nepochs,lr_schedule,prunepochs,sparsities,base_level_=0.1):
        ...
        for epoch in range(self.first_epoch,nepochs):
            ...
            if epoch in prunepochs and (not self.distributed or self.rank == 0):
                ...
                if epoch_index > 0:
                    ...
                    base_level = base_level_
                self.model.eval()
                w_pruned,mask=self.pruner.prune_group(self.mask,sparsities[epoch_index],base_level)
                self.mask = mask.to(self.device)
                del mask
                del w_pruned
                pruning_res = self.pruner.results
