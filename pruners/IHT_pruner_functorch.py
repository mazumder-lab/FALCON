...
class IHTCDLSPruner_functorch:
    ...
    device,algo='Active_IHTCDLS'):
        ...
        self.fmodel, self.fparams, self.buffers = make_functional_with_buffers(self.model)

        def compute_loss_stateless_model (fparams, buffers, sample, target):
            batch = sample.unsqueeze(0)
            targets = target.unsqueeze(0)

            predictions = self.fmodel(fparams, buffers, batch) 
            loss = self.criterion(predictions, targets)
            return loss
        ft_compute_grad = grad(compute_loss_stateless_model)
        self.ft_compute_sample_grad = vmap(ft_compute_grad, in_dims=(None, None, 0, 0))
    ...
    def compute_sample_grads(self,x,y):
        ft_per_sample_grads = self.ft_compute_sample_grad(self.fparams, self.buffers, x, y)
        grads = []
        for i,par in enumerate(self.model.named_parameters()):
            par_name = par[0]
            if par_name in self.params:
                grads.append(ft_per_sample_grads[i].reshape(ft_per_sample_grads[i].shape[0],-1))
    ...
    def prune(self,mask,sparsity,grads=None):
        for i, batch in enumerate(self.prun_dataloader):
            ...
            x = x.to(self.device)
            y = y.to(self.device)
            grads = self.compute_sample_grads(x,y)
            self.ngrads = grads.shape[0]
            self.lambda2 = self.l2*self.ngrads/2
