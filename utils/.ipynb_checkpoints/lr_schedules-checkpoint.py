...
def cosine_lr_restarts(epochs,max_lr,min_lr,n_prune_epochs,prune_every,gamma_ft=-1,warm_up=0):
    lr_list =[max_lr]*warm_up
    epochs -= warm_up
    for _ in range(n_prune_epochs-1):
        lr_list += cosine_lr(prune_every,max_lr,min_lr)
    if gamma_ft < 0:
        lr_list += cosine_lr(epochs - prune_every*(n_prune_epochs-1),max_lr,min_lr)
    else:
        lr_list += [max_lr*np.power(gamma_ft,i)  for i in range(epochs - prune_every*(n_prune_epochs-1)) ]
    return lr_list
